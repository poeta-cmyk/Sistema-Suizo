...
# CSS: Mesas de Gran Tamaño y Alta Visibilidad
st.markdown("""
    <style>
    .mesa-container {
        display: flex;
...
if 'asistentes' not in st.session_state:
    st.session_state.update({
        ...
        'meta_torneo': 100, 'seccion_activa': "INSCRIPCIÓN"
    })
...
if st.session_state.seccion_activa == "INSCRIPCIÓN":
    ...
    # Botón con el texto exacto solicitado por el usuario
    if st.button("🚀 SORTEAR E IR A RONDA 1"):
        if len(activos) >= 4:
            random.shuffle(activos)
            n_jug = (len(activos) // 4) * 4
            mesas_generadas = [activos[i:i+4] for i in range(0, n_jug, 4)]
            st.session_state.historial_completo.append({
                'ronda': 1, 'mesas': mesas_generadas, 'reposo': activos[n_jug:], 'listas': []
            })
...
# --- 4. SECCIÓN: MESAS (GRAN TAMAÑO) ---
    ...
    if st.session_state.historial_completo:
        ...
        if r['reposo']:
            st.info(f"💤 REPOSO: {', '.join(r['reposo'])}")
        ...
        for i, m in enumerate(r['mesas']):
            if i not in r['listas']:
                with st.expander(f"MESA {i+1}: {m[0]}/{m[2]} vs {m[1]}/{m[3]}", expanded=True):
                    c1, c2 = st.columns(2)
                    v1 = c1.number_input(f"Pareja A-C:", 0, 250, key=f"v1_{i}")
                    v2 = c2.number_input(f"Pareja B-D:", 0, 250, key=f"v2_{i}")
