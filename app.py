...
# CSS: Diseño Institucional ADEL y Alertas
st.markdown("""
    <style>
    ...
    if st.button("🚀 REALIZAR SORTEO E IR A MESAS"):
        if len(activos) >= 4:
            ...
            st.session_state.historial_completo.append({
                ...
                'listas': [],
                'reposo_procesado': False # Flag para evitar duplicar victorias
            })
            st.session_state.seccion_active = "MESAS"
            st.rerun()
...
# --- 5. SECCIÓN: RESULTADOS (Con Lógica de Reposo) ---
elif st.session_state.seccion_activa == "RESULTADOS":
    st.header("Carga de Resultados")
    if st.session_state.historial_completo:
        r = st.session_state.historial_completo[-1]
        
        # Procesar victorias automáticas por reposo una sola vez
        ...
        # Mostrar quiénes ganaron por reposo
        if r.get('reposo'):
            for p in r['reposo']:
                st.markdown(f'<div class="victoria-reposo">🏆 {p}: Gana por Reposo (+1 JJ)</div>', unsafe_allow_html=True)
            st.divider()

        # Carga de mesas activas
        for i, m in enumerate(r['mesas']):
            if i not in r['listas']:
                with st.expander(f"MESA {i+1}", expanded=True):
                    c1, c2 = st.columns(2)
                    v1 = c1.number_input(f"Pareja A-C ({m[0]}/{m[2]}):", 0, 200, key=f"v1_{i}")
                    v2 = c2.number_input(f"Pareja B-D ({m[1]}/{m[3]}):", 0, 200, key=f"v2_{i}")
                    if st.button(f"GUARDAR RESULTADO MESA {i+1}", key=f"b_{i}"):
                        for j in [m[0], m[2]]:
